...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg, output_shape):
        ...
        print("Backbone output_shape:", output_shape)
...
class CondInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.mask_head = DynamicMaskHead(cfg, self.backbone.output_shape())
        ...
        in_channels = self.backbone.output_shape()["p5"].channels
        
        self.roi_pooler = ROIPooler(
            output_size = 1,  # (1,1) RoI pooling -> 벡터 하나로 
            scales = (1.0/32,),  # p5 feature map은 stride 32
            sampling_ratio = 0,
    ...
    def forward_teacher(self, batched_inputs):
        ...
        with torch.no_grad():
            ...
            # 2. list로 나온 결과를 .tensor로 변환 
            images_norm = ImageList.from_tensors(images_norm, self.backbone.size_divisibility)
            
            # 3. Feature 추출
            features = self.backbone(images_norm.tensor)
        
            # 4. mask branch feature 추출출
            gt_instances = None  # inference mode (파인튜닝인데도... 필요한가...?)
            mask_feats, _ = self.mask_branch(features, gt_instances)

            # 5. Proposal 생성
            proposals,_ = self.proposal_generator(images_norm, features, gt_instances=None)
            #proposals, top_feats = self.proposal_generator(
            #images_norm, features, gt_instances=None)
                        
            # 6. Dummy top_feats 생성
            proposal_box = [p.pred_boxes for p in proposals]
            feat_map = self.controller(features["p5"])
            
            top_feats = self.roi_pooler([feat_map], proposal_box)
            top_feats = top_feats.view(top_feats.size(0), -1)
            
            start = 0
            for p in proposals:
                num = len(p)
                p.set("top_feat", top_feats[start:start+num])
                start += num
            ...
            # for im_id, (proposal, image_size) in enumerate(zip(proposals, image_sizes)):
            for im_id, (proposal, image_size) in enumerate(zip(proposals, image_sizes)):

                height, width = image_size
                
                proposal.mask_head_params = proposal.top_feat
                
                instances_per_im = self._forward_mask_heads_test(
                    [proposal], mask_feats[im_id].unsqueeze(0))

                teacher_threshold = -1.0 if self.return_float_masks else self.teacher_mask_threshold
    ...
    def forward(self, batched_inputs):
        ...
        proposal_instances = proposals["instances"]
        
        if not (proposal_instances.has("proposal_boxes") or proposal_instances.has("pred_boxes")):
            N = len(proposal_instances)
            device = next(iter(proposal_instances.get_fields().values())).device
            dummy_boxes = torch.zeros((N, 4), device=device)
            proposal_instances.set("proposal_boxes", Boxes(dummy_boxes))
            
        num_images = images_norm.tensor.shape[0]
        proposal_instances_per_image = [
            proposal_instances[proposal_instances.im_inds == i] for i in range(num_images)
        ]
        proposal_boxes = [
            inst.proposal_boxes if inst.has("proposal_boxes") 
            else inst.pred_boxes if inst.has("pred_boxes")
            else Boxes(torch.zeros((len(inst), 4), device=next(iter(inst.get_fields().values())).device))
            for inst in proposal_instances_per_image
        ]

        top_feat_map = self.controller(features["p5"])
        top_feats = self.roi_pooler([top_feat_map], proposal_boxes)
        top_feats = top_feats.view(top_feats.size(0), -1)
        proposal_instances.set("top_feat", top_feats)
